...
export async function applyWalletFunding(
  ...
  // Calculate total balance from all user wallets (single source of truth)
  ...
  if (allWalletsError) {
    throw new Error(allWalletsError.message)
  }

  // Convert all balances to USD for global balance
  ...
  const newGlobalBalance = (allWallets ?? []).reduce((sum, w) => {
